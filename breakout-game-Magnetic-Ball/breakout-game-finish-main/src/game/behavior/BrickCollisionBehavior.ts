...
export class BrickCollisionBehavior extends GameObjectBehavior {
    ...
    private wasHit() {
        ...
        if(this.brickType === 'TYPE_1'&&Math.random()<0.5){EventDispatcher.getInstance().getDispatcher().emit(GameEvents.MAGNETIC_BALL, {objId: this.gameObjRef.getId(), brickType: this.brickType, nbrHit: this.totalNbrHit});
    }
        this.isActive = false;
